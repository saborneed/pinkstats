...
server <- function(input, output, session) {
  ...
  output$histPlot <- renderPlot({
    ...
    if (is.null(df)) {
      ...
      gg + labs(title = paste0("Histogram of S(t) — last run: nsim=", last_run_meta()$nsim), x = "S(t)", y = "Density") + theme_minimal() + theme(plot.title = element_text(color = "#3B1F2B", face = "bold"))
    }
  })
